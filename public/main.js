...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (window.UIManager && window.EventManager) {
        ...
        // Debug shortcut
        window.triggerEvent = (charId, rank) => window.eventManager.triggerEvent(charId, rank);
    } else {
    ...
    window.addEventListener('settingChanged', (e) => {
        if (e.detail.type === 'textSpeed') {
            switch(e.detail.value) {
                case 'slow': typeSpeedMultiplier = 2.0; break;
                case 'fast': typeSpeedMultiplier = 0.5; break;
                case 'instant': typeSpeedMultiplier = 0; break;
                case 'normal': default: typeSpeedMultiplier = 1.0; break;
            }
    ...
    document.body.addEventListener('mouseover', (e) => {
        if (e.target.matches('button, .btn-premium, .btn-console, input[type="range"], .settings-tab, .item-entry')) {
    ...
    const headerProgressionToggle = document.getElementById("header-progression-toggle");
    ...
    if (headerProgressionToggle) {
        headerProgressionToggle.addEventListener("change", (e) => {
            ...
            log(`System: Progression mode set to ${isAutoProgression ? 'AUTO' : 'MANUAL'}`);
            if (elements.audioManager) elements.audioManager.playSE('click');
        });
    }
    ...
    if (elements.bootScreen) {
        elements.bootScreen.addEventListener("click", () => {
            ...
            if (elements.backgroundLayer) elements.backgroundLayer.classList.add("bg-title");
    ...
    function showCharacterCreation() {
        return new Promise(resolve => {
            ...
            btnConfirmCreation.onclick = () => {
                ...
                const forbidden = ["Black Knight", "Schwarz", "黒騎士", "ブラックナイト", "Noir"];
                if (forbidden.some(term => name.toLowerCase().includes(term.toLowerCase()))) {
                     creationMessage.textContent = "FATAL ERROR: IDENTITY_RECURSION_DETECTED";
                ...
                log(`Created Character: ${name}, Stats: ${JSON.stringify(playerStats)}`);
    ...
    async function renderQuestion() {
        ...
        await typeWriter(`[ Question ${progress.current} / ${progress.total} ]\n${q.text}`, questionText, 20);
    ...
    async function finishEvaluation() {
        ...
        await typeWriter("「あなたの好きな言葉は何ですか？ 物でも概念でも人の名前でも、何でもいいです。」\n「……教えてくれますか？」", questionText);
