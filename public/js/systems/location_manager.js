...
class LocationManager {
  constructor() {
    ...
    this.locations = {
      ...
      base_hall: {
        ...
        destinations: ["lower_main_street", "lower_market", "lower_alley"], // Fixed IDs
        actions: [
          {
            id: "rest",
            name: "休息する",
            icon: "💤",
            condition: (gs) => gs.player.hp < gs.player.maxHp,
          },
      ...
      crows_nest_room: {
        ...
        connections: ["base_hall"], // Fixed: crows_nest_hall -> base_hall
        travelTime: 0,
        encounterRate: 0,
        shopType: null,
        canRest: true,
  ...
  toggleExpand() {
    console.log(
      "[LocationManager] toggleExpand called, isExpanded:",
      this.isExpanded,
    );
    ...
    console.log(
      "[LocationManager] toggleExpand done, isExpanded:",
      this.isExpanded,
    );
  }
  ...
  async moveTo(destinationId) {
    ...
    if (typeof battleSystem !== "undefined" && !battleSystem.inBattle) {
      if (
        typeof checkEncounter !== "undefined" &&
        checkEncounter(destinationId, isNight)
      ) {
        // 敵を抽選
        const enemyData =
          typeof spawnEnemy !== "undefined"
            ? spawnEnemy(destinationId, isNight)
            : null;
  ...
  grantStarterGear() {
    ...
    if (
      window.audioManager &&
      typeof window.audioManager.playSE === "function"
    ) {
